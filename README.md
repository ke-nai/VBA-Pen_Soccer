...
펜축구는 양 플레이어가 번갈아 가며 선을 그어 상대방의 골대로 들어가야하는 게임이다.

이때, 이미 지나간 점을 지날 때는 이동할 기회가 연장되나 이미 지나간 길은 지나갈 수 없기 때문에

이 룰을 이용해 상대방의 진행을 막으며 나는 이동 횟수를 늘려 상대방의 골대로 가야한다.

## 구현한 핵심 알고리즘
1. 이전 상태에 따른 다음 가능한 선택지 판단
2. 이동 경로에 따른 턴 넘김 판단
